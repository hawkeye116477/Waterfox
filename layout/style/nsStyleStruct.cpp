...
static bool
DefinitelyEqualURIs(css::URLValueData* aURI1, css::URLValueData* aURI2)
{
  return aURI1 == aURI2 ||
...
static bool
AreShadowArraysEqual(nsCSSShadowArray* lhs, nsCSSShadowArray* rhs);
...
nsStyleFont::nsStyleFont(const nsPresContext* aContext)
  : nsStyleFont(
      *aContext->GetDefaultFont(kPresContext_DefaultVariableFont_ID, nullptr),
      aContext)
{}
...
nsStyleFont::CalcDifference(const nsStyleFont& aNewData) const
{
  ...
  if (mSize != aNewData.mSize || mFont != aNewData.mFont ||
...
nsStyleSVGPaint::Assign(const nsStyleSVGPaint& aOther)
{
  ...
  switch (aOther.mType) {
    ...
    case eStyleSVGPaintType_ContextStroke:
      SetContextValue(
        aOther.mType, aOther.mFallbackType, aOther.mFallbackColor);
      break;
  }
}
...
bool
nsStyleSVGPaint::operator==(const nsStyleSVGPaint& aOther) const
{
  if (mType != aOther.mType) {
    return false;
  }
...
nsStyleImageRequest::Resolve(nsPresContext* aPresContext)
{
  ...
  mRequestProxy =
    value.GetPossiblyStaticImageValue(aPresContext->Document(), aPresContext);
...
static inline bool
EqualRects(const UniquePtr<nsStyleSides>& aRect1,
...
bool
nsStyleImageLayers::operator==(const nsStyleImageLayers& aOther) const
{
...
bool
nsStyleImageLayers::IsInitialPositionForLayerType(Position aPosition,
                                                  LayerType aType)
{
  ...
  if (type == eStyleImageType_Image) {
    ...
    if (imgContainer) {
      ...
      nsLayoutUtils::ComputeSizeForDrawing(
        imgContainer, imageSize, imageRatio, hasWidth, hasHeight);
...
nsStyleDisplay::nsStyleDisplay(const nsPresContext* aContext)
  ...
  , mTransformOrigin{ { 0.5f,
                        eStyleUnit_Percent }, // Transform is centered on origin
...
nsStyleDisplay::~nsStyleDisplay()
{
  ...
  if (mSpecifiedTransform && ServoStyleSet::IsInServoTraversal()) {
    ...
    bool alwaysProxy =
      ...
      false;
#endif
    NS_ReleaseOnMainThreadSystemGroup("nsStyleDisplay::mSpecifiedTransform",
                                      mSpecifiedTransform.forget(),
                                      alwaysProxy);
  }
...
nsStyleDisplay::CalcDifference(const nsStyleDisplay& aNewData) const
{
  ...
  if (!DefinitelyEqualURIsAndPrincipal(mBinding.ForceGet(),
                                       aNewData.mBinding.ForceGet()) ||
...
bool
nsStyleContentData::CounterFunction::operator==(
...
nsStyleUIReset::~nsStyleUIReset()
{
  ...
  if (mSpecifiedWindowTransform && ServoStyleSet::IsInServoTraversal()) {
    ...
    NS_ReleaseOnMainThreadSystemGroup(
      ...
      mSpecifiedWindowTransform.forget(),
      alwaysProxy);
  }
}
