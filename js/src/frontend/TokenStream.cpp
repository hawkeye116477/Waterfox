...
TokenStreamSpecific<CharT, AnyCharsAccess>::getTokenInternal(TokenKind* ttp, Modifier modifier)
{
  ...
  retry:
    ...
    if (c1kind == Dec) {
      ...
      decimal_rest:
        ...
        if (!((decimalPoint == HasDecimal) || hasExp)) {
            ...
            if (!GetDecimalNonInteger(anyCharsAccess().cx, numStart, userbuf.addressOfNextRawChar(), &dval))
            {
                goto error;
            }
        }
    ...
    if (c1kind == BasePrefix) {
        ...
        if (!GetPrefixInteger(anyCharsAccess().cx, numStart, userbuf.addressOfNextRawChar(), radix,
