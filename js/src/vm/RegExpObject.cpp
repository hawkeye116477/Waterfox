...
RegExpObject* RegExpObject::create(JSContext* cx, HandleAtom source,
                                   RegExpFlags flags, LifoAlloc& alloc,
                                   NewObjectKind newKind) {
...
js::XDRScriptRegExpObject(XDRState<mode>* xdr, MutableHandle<RegExpObject*> objp)
{
    ...
    if (mode == XDR_DECODE) {
        ...
        if (xdr->hasOptions()) options = &xdr->options();
        RegExpObject* reobj =
            RegExpObject::create(xdr->cx(), source, RegExpFlags(flags),
...
JS::NewRegExpObject(JSContext* cx, const char* bytes, size_t length, RegExpFlags flags)
{
    ...
    return RegExpObject::create(
      cx, chars.get(), length, flags, cx->tempLifoAlloc(), GenericObject);
}
...
JS::NewUCRegExpObject(JSContext* cx, const char16_t* chars, size_t length, RegExpFlags flags)
{
    ...
    CHECK_REQUEST(cx);
    return RegExpObject::create(
      cx, chars, length, flags, cx->tempLifoAlloc(), GenericObject);
}
